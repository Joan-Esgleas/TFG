...
class VerilatorSim:
    # pylint: disable-next=consider-using-with
    logfile = tempfile.NamedTemporaryFile(prefix='sim', suffix='.log')
    logname = logfile.name
    lognames = [logname]

    def __init__(self, sim_cmd=None, debug=False, jtag_flag=True, timeout=300):
        real_stdout.write(f"Init Verilator\n")
        if sim_cmd:
        ...
        if debug:
            cmd += ["+vcd"]
        real_stdout.write(f"Verilator command: {cmd}\n")
        # pylint: disable-next=consider-using-with
        logfile = open(self.logname, "w", encoding='utf-8')
        if print_log_names:
            real_stdout.write(f"Temporary Verilator log: {self.logname}\n")
...
class QuestaSim:
    ...
    def __init__(self, sim_cmd=None, debug=False, jtag_flag=True, timeout=300):
        real_stdout.write(f"Init Questa\n")
        if sim_cmd:
            cmd = shlex.split(sim_cmd)
        else:
            cmd = ["./sim"]
        ...
        if debug:
            cmd += ["+vcd"]
        real_stdout.write(f"Questa command: {cmd}\n")
        # pylint: disable-next=consider-using-with
        logfile = open(self.logname, "w", encoding='utf-8')
        if print_log_names:
            real_stdout.write(f"Temporary Questa log: {self.logname}\n")
        logfile.write("+ " + " ".join(cmd) + "\n")
        logfile.flush()
        ...
        with open(self.logname, "r", encoding='utf-8') as listenfile:
            ...
            while not done:
                ...
                if exit_code is not None:
                    raise RuntimeError(
                        f'Questa simulator exited early with status {exit_code}')
                ...
                if (time.time() - start) > timeout:
                    raise TestLibError(
                        "Timed out waiting for Questa to listen for JTAG vpi")
...
class FPGAEmulation:
    ...
    def __init__(self, sim_cmd=None, debug=False, timeout=300):
        real_stdout.write(f"Load binary to fpga and reset the design\n")
        if sim_cmd:
            cmd = shlex.split(sim_cmd)
        else:
            raise TestLibError(
                        "Invalid command for loading a binary to FPGA")

        real_stdout.write(f"Load command: {cmd}\n")
        # pylint: disable-next=consider-using-with
        logfile = open(self.logname, "w", encoding='utf-8')
        if print_log_names:
            real_stdout.write(f"Temporary FPGA log: {self.logname}\n")
        logfile.write("+ " + " ".join(cmd) + "\n")
        logfile.flush()
            ...
            while not done:
                ...
                if exit_code is not None:
                    if exit_code == 0:
                        ...
                        raise RuntimeError(
                            f'FPGA load exited early with status {exit_code}')

                line = listenfile.readline()
                if not line:
                    time.sleep(1)
                match = re.match(r"^Removing ./osbi.bin image", line)
                if match:
                    done = True

                if (time.time() - start) > timeout:
                    raise TestLibError(
                        "Timed out waiting for FPGA to listen for JTAG vpi")
...
class GdbTest(BaseTest):
    ...
    def postMortem(self):
        if not self.gdb:
            return
        if self.target.no_post_mortem:
            print("Skipping post_mortem")
            return
        self.gdb.interrupt()
