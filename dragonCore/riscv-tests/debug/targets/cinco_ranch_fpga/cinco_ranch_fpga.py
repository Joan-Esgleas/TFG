...
class sargantana_hart(targets.Hart):
    ...
    link_script_path = "cinco_ranch_fpga.lds"
    misa = 0x8000000000341129
    honors_tdata1_hmode = False

class cinco_ranch_fpga(targets.Target):
    harts = [sargantana_hart()]
    openocd_config_path = "cinco_ranch_fpga.cfg"
    timeout_sec = 5
    ...
    #def create(self):
    #    return testlib.FPGAEmulation(sim_cmd=self.sim_cmd, debug=False,timeout=60)